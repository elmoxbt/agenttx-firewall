...
export type Parsed =
  ...
  | { type: "sol_create_account"; from: string; newAccount: string; lamports: bigint; space: bigint; owner: string }
  | { type: "assign"; account: string; owner: string }
  | { type: "token_transfer"; source: string; mint?: string; destination: string; authority: string; amount: bigint; decimals?: number }
  | { type: "token_approve"; source: string; mint?: string; delegate: string; owner: string; amount: bigint; decimals?: number }
  | { type: "token_revoke"; source: string; owner: string }
  | { type: "authority_change"; target: string; kind: string; newAuthority: string | null; currentAuthority: string }
  | { type: "token_close"; account: string; destination: string; owner: string }
  | { type: "token_burn"; account: string; authority: string; amount: bigint }
  | { type: "token_mint"; mint: string; destination: string; authority: string; amount: bigint }
  | { type: "token_sync_native"; account: string }
  | { type: "ata_create"; payer: string; ata: string; owner: string; mint: string; tokenProgram: string; idempotent: boolean }
  | { type: "compute_limit"; units: number }
  | { type: "compute_price"; microLamports: bigint }
  | { type: "memo"; text: string }
  | {
      type: "swap_route";
      protocol: "jupiter";
      mode: "exact_in" | "exact_out";
      /** exact_in: input amount. exact_out: output amount. */
      amount: bigint;
      /** exact_in: quoted output. exact_out: quoted (max) input. */
      otherAmount: bigint;
      slippageBps: number;
...
export interface DecodedInstruction {
  ...
  programId: string;
  programLabel: string;
  /** snake_case instruction name, or `disc:<hex>` for unrecognised Anchor-style payloads. */
  name: string;
  /** First 8 bytes (hex) for programs decoded as Anchor-style; used to match named allowlist entries. */
...
function need(ix: Instruction, minData: number, minAccounts: number): void {
  if (ix.data.length < minData) throw new Malformed(`data too short (${ix.data.length} < ${minData})`);
  if (ix.accounts.length < minAccounts) {
    throw new Malformed(`expected at least ${minAccounts} accounts, got ${ix.accounts.length}`);
  }
}

const acc = (ix: Instruction, i: number): string => ix.accounts[i].pubkey;
const pk = (d: Uint8Array, o: number): string => encodeBase58(d.subarray(o, o + 32));
...
// System program
// ---------------------------------------------------------------------------------------------
function decodeSystem(ix: Instruction): Decoded {
  need(ix, 4, 0);
  const d = ix.data;
  ...
  switch (kind) {
    ...
    case 3: {
      ...
      if (seedLen > 32) throw new Malformed("seed too long");
      const o = 44 + seedLen;
      need(ix, o + 48, 2);
      return {
...
function decodeToken(ix: Instruction): Decoded {
  need(ix, 1, 0);
  const d = ix.data;
  switch (d[0]) {
    case 0:
...
function decodeAta(ix: Instruction): Decoded {
  ...
  if (kind === 2) return { name: "recover_nested", parsed: { type: "opaque" } };
  return { name: `unknown_${kind}`, parsed: { type: "opaque" } };
}

function decodeComputeBudget(ix: Instruction): Decoded {
  need(ix, 1, 0);
  const d = ix.data;
  switch (d[0]) {
    case 1:
...
function decodeJupiter(ix: Instruction): Decoded {
  need(ix, 8, 0);
  const d = ix.data;
  const disc = toHex(d.subarray(0, 8));
  const name = JUPITER_BY_DISC.get(disc);
  if (!name) return { name: `disc:${disc}`, discriminator: disc, parsed: { type: "opaque" } };
  if (!JUPITER_ROUTES.has(name)) return { name, discriminator: disc, parsed: { type: "other" } };

  need(ix, 8 + 19, 0);
  const n = d.length;
  const shared = name.startsWith("shared_accounts");
  const a = ix.accounts;
  const at = (i: number): string | undefined => a[i]?.pubkey;
  let authority: string | undefined;
  let userSource: string | undefined;
  let destination: string | undefined;
  let sourceMint: string | undefined;
  let destinationMint: string | undefined;
  if (shared) {
...
function decodeAnchorOpaque(ix: Instruction): Decoded {
  if (ix.data.length >= 8) {
...
export function decodeInstruction(ix: Instruction, index: number): DecodedInstruction {
...
export function decodeAll(instructions: Instruction[]): DecodedInstruction[] {
  return instructions.map((ix, i) => decodeInstruction(ix, i));
}

/** One-line description used in reports. */
export function describeInstruction(d: DecodedInstruction): string {
  const p = d.parsed;
