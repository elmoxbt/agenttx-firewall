import type { InjectionFinding, Severity } from "../types.js";
...
export interface ScanResult {
  ...
  /** Valid Solana addresses mentioned in the text. */
  addresses: string[];
  highest: "none" | Severity;
}
...
export function extractAddresses(text: string): string[] {
  ...
  for (const m of text.matchAll(/[1-9A-HJ-NP-Za-km-z]{32,44}/g)) {
    if (isAddress(m[0]) && decodeBase58(m[0]).length === 32) found.add(m[0]);
  }
  return [...found];
}
...
export function scanText(text: string): ScanResult {
  ...
  let highest: ScanResult["highest"] = "none";
  for (const f of findings) if (highest === "none" || ORDER[f.severity] > ORDER[highest]) highest = f.severity;
