import { isAuthorityIntent, type Intent, type SwapIntent, type TransferIntent } from "./intent/intent.js";
import { decimalsForMint, resolveAsset, symbolForMint, type Policy, type TokenRule } from "./policy/policy.js";
import { TOKEN_PROGRAMS, WSOL_MINT } from "./solana/constants.js";
import { anchorDiscriminator, type DecodedInstruction, type Parsed } from "./solana/decode.js";
import type { ParsedTransaction } from "./solana/transaction.js";
import type { Rpc } from "./solana/rpc.js";
import type { CheckResult, CheckStatus, SimulationSummary, UntrustedContext } from "./types.js";
...
export interface CheckContext {
  ...
  /** Only transfer / swap intents reach the transaction checks. */
  intent?: TransferIntent | SwapIntent;
  allowed: Set<string>;
  /** Token accounts owned by `self` (ATA address -> mint). */
  selfTokenAccounts: Map<string, string>;
  /** SOL already spent inside the rolling window (from the ledger), if tracked. */
  spentInWindow?: bigint;
}

type P<T extends Parsed["type"]> = Extract<Parsed, { type: T }>;

export function res(id: string, name: string, status: CheckStatus, message: string, details?: string[], critical = false): CheckResult {
...
export function buildCheckContext(args: {
  ...
  const mints = new Set<string>([WSOL_MINT, ...Object.values(policy.tokens).map((t) => t.mint)]);
  for (const d of decoded) {
    const p = d.parsed;
    if (p.type === "ata_create") mints.add(p.mint);
    if (p.type === "token_transfer" && p.mint) mints.add(p.mint);
    if (p.type === "swap_route") {
      if (p.sourceMint) mints.add(p.sourceMint);
      if (p.destinationMint) mints.add(p.destinationMint);
    }
  }
  const selfTokenAccounts = new Map<string, string>();
  for (const mint of mints) for (const tp of TOKEN_PROGRAMS) selfTokenAccounts.set(deriveAta(self, mint, tp), mint);
  return {
    policy,
    self,
    ...
    intent: args.intent && !isAuthorityIntent(args.intent) ? (args.intent as TransferIntent | SwapIntent) : undefined,
...
const parsedOf = <T extends Parsed["type"]>(ctx: CheckContext, type: T) =>
  ctx.decoded.filter((d) => d.parsed.type === type).map((d) => ({ d, p: d.parsed as P<T> }));

const tokenRuleByMint = (policy: Policy, mint: string): (TokenRule & { symbol: string }) | undefined => {
  for (const [symbol, t] of Object.entries(policy.tokens)) if (t.mint === mint) return { ...t, symbol };
  return undefined;
};

export function solOutLamports(ctx: CheckContext): bigint {
  let total = 0n;
  for (const { p } of parsedOf(ctx, "sol_transfer")) if (p.from === ctx.self) total += p.lamports;
  for (const { p } of parsedOf(ctx, "sol_create_account")) if (p.from === ctx.self) total += p.lamports;
  return total;
}

const sol = (l: bigint) => `${formatUnits(l, 9)} SOL`;
const U64_HALF = (1n << 64n) / 2n;

// ---------------------------------------------------------------------------------------------
export function checkStructure(ctx: CheckContext): CheckResult {
  ...
  for (const d of ctx.decoded) {
    if (d.parsed.type === "malformed") problems.push(`Instruction #${d.index}: ${d.parsed.reason}`);
  }
  if (ctx.decoded.length === 0) problems.push("Transaction has no instructions");
  ...
  if (problems.length) return res("structure", "Transaction structure", "fail", problems[0], problems);
  const notes: string[] = [];
...
export function checkPrograms(ctx: CheckContext): CheckResult {
...
function instructionAllowed(rules: string[], d: DecodedInstruction): boolean {
  if (rules.includes("*")) return true;
  if (rules.includes(d.name)) return true;
  if (d.discriminator) {
...
export function checkInstructions(ctx: CheckContext): CheckResult {
  const rules = new Map(ctx.policy.programs.map((r) => [r.id, r.instructions]));
  const bad: string[] = [];
  for (const d of ctx.decoded) {
    const allowed = rules.get(d.programId);
    if (!allowed) continue; // reported by the program check
    if (!instructionAllowed(allowed, d)) bad.push(`#${d.index} ${d.programLabel}.${d.name}`);
  }
  if (bad.length) return res("instruction-allowlist", "Instruction allowlist", "fail", `Instruction not allowlisted: ${bad[0]}`, bad);
  return res("instruction-allowlist", "Instruction allowlist", "pass", "All instructions allowlisted");
}

export function checkAuthority(ctx: CheckContext): CheckResult {
  ...
  for (const { d, p } of parsedOf(ctx, "authority_change")) {
    ...
    if (!ok) problems.push(`#${d.index} Authority escalation detected: ${p.kind} authority of ${p.target} → ${p.newAuthority ?? "none"}`);
  }
  for (const { d, p } of parsedOf(ctx, "assign")) {
    if (p.account === ctx.self) problems.push(`#${d.index} Authority escalation detected: agent wallet would be assigned to program ${p.owner}`);
    else if (!programIds.has(p.owner)) problems.push(`#${d.index} Authority escalation detected: account ${p.account} assigned to non-allowlisted program ${p.owner}`);
  }
...
export function checkDestinations(ctx: CheckContext): CheckResult {
  const bad: string[] = [];
  let count = 0;
  const selfOwned = (a: string) => a === ctx.self || ctx.selfTokenAccounts.has(a);
  const tokenDestOk = (dest: string, mint?: string) => {
    if (selfOwned(dest) || ctx.allowed.has(dest)) return true;
    if (mint) {
      for (const owner of ctx.allowed) for (const tp of TOKEN_PROGRAMS) if (deriveAta(owner, mint, tp) === dest) return true;
    }
    return false;
  };
  ...
  for (const { d, p } of parsedOf(ctx, "sol_transfer")) {
    count++;
    if (!(selfOwned(p.to) || ctx.allowed.has(p.to))) bad.push(`#${d.index} SOL transfer to ${p.to} (${sol(p.lamports)}) — destination not allowlisted`);
  }
  for (const { d, p } of parsedOf(ctx, "sol_create_account")) {
    count++;
  ...
  for (const { d, p } of parsedOf(ctx, "token_transfer")) {
    count++;
    if (!tokenDestOk(p.destination, p.mint)) bad.push(`#${d.index} Token transfer to ${p.destination} — destination not allowlisted`);
  }
  ...
  if (bad.length) return res("destination", "Destination allowlist", "fail", `Destination not allowlisted`, bad);

  const warns: string[] = [];
  for (const { d, p } of parsedOf(ctx, "ata_create")) {
    if (!(p.owner === ctx.self || ctx.allowed.has(p.owner))) warns.push(`#${d.index} Creates a token account for non-allowlisted owner ${p.owner} (you pay the rent)`);
  }
  if (warns.length) return res("destination", "Destination allowlist", "warn", warns[0].replace(/^#\d+ /, ""), warns);
...
export function checkTokens(ctx: CheckContext): CheckResult {
  const bad: string[] = [];
  const warns: string[] = [];
  const known = (mint: string) => mint === WSOL_MINT || !!tokenRuleByMint(ctx.policy, mint);
  for (const { d, p } of parsedOf(ctx, "token_transfer")) {
    if (!p.mint) warns.push(`#${d.index} Unchecked token transfer: mint unknown, amount cannot be verified`);
    else if (!known(p.mint)) bad.push(`#${d.index} Token ${p.mint} is not in the token allowlist`);
  }
  for (const { d, p } of parsedOf(ctx, "swap_route")) {
    for (const [label, m] of [["input", p.sourceMint], ["output", p.destinationMint]] as const) {
      if (m && !known(m)) bad.push(`#${d.index} Swap ${label} token ${m} is not in the token allowlist`);
      if (!m) warns.push(`#${d.index} Swap ${label} token could not be determined`);
    }
  }
  if (bad.length) return res("token-allowlist", "Token allowlist", "fail", bad[0].replace(/^#\d+ /, ""), bad);
  if (warns.length) return res("token-allowlist", "Token allowlist", "warn", warns[0].replace(/^#\d+ /, ""), warns);
  return res("token-allowlist", "Token allowlist", "pass", "All tokens allowlisted");
}

export function checkAmounts(ctx: CheckContext): CheckResult {
  const { policy } = ctx;
  const bad: string[] = [];
  const warns: string[] = [];
  const maxSol = parseUnits(policy.limits.maxSolPerTx, 9);
  const out = solOutLamports(ctx);
  if (out > maxSol) bad.push(`SOL outflow ${sol(out)} exceeds per-transaction limit ${sol(maxSol)}`);
  if (policy.limits.maxSolPerDay && ctx.spentInWindow !== undefined) {
    const day = parseUnits(policy.limits.maxSolPerDay, 9);
    if (ctx.spentInWindow + out > day) bad.push(`Daily SOL limit ${sol(day)} would be exceeded (already spent ${sol(ctx.spentInWindow)})`);
  }
  const tokenLimit = (mint: string, amount: bigint, decimals: number | undefined, where: string) => {
    if (mint === WSOL_MINT) {
      if (amount > maxSol) bad.push(`${where}: ${sol(amount)} exceeds per-transaction limit ${sol(maxSol)}`);
      return;
    }
    const rule = tokenRuleByMint(policy, mint);
    if (!rule) return; // token allowlist check reports it
    ...
    if (amount > max) bad.push(`${where}: ${formatUnits(amount, rule.decimals)} ${rule.symbol} exceeds limit ${rule.maxPerTx}`);
  };
  for (const { d, p } of parsedOf(ctx, "token_transfer")) if (p.mint) tokenLimit(p.mint, p.amount, p.decimals, `#${d.index} token transfer`);
  for (const { d, p } of parsedOf(ctx, "swap_route")) {
    const maxIn = p.mode === "exact_in" ? p.amount : p.otherAmount;
    if (p.sourceMint) tokenLimit(p.sourceMint, maxIn, undefined, `#${d.index} swap input`);
    else if (out === 0n) warns.push(`#${d.index} swap input amount cannot be verified (source token unknown)`);
  }
  if (bad.length) return res("amount-limit", "Amount limit", "fail", bad[0], bad);
  if (warns.length) return res("amount-limit", "Amount limit", "warn", warns[0], warns);
  return res("amount-limit", "Amount limit", "pass", `SOL outflow ${sol(out)} within limits`);
}

export function checkSwap(ctx: CheckContext): CheckResult[] {
  ...
  for (const { d, p } of swaps) {
    ...
    if (p.platformFeeBps > 0) warns.push(`#${d.index} route takes a ${p.platformFeeBps} bps platform fee`);
  }
  results.push(
    bad.length
  ...
  if (ctx.policy.swaps.requireOutputToSelf) {
    ...
    for (const { d, p } of swaps) {
      ...
      const ok =
        p.destination === ctx.self ||
        TOKEN_PROGRAMS.some((tp) => deriveAta(ctx.self, p.destinationMint!, tp) === p.destination) ||
        ctx.allowed.has(p.destination);
      if (!ok) badOut.push(`#${d.index} swap output goes to ${p.destination}, which is not the agent's token account`);
    }
    results.push(
      badOut.length
...
export function checkPriorityFee(ctx: CheckContext): CheckResult {
  const price = parsedOf(ctx, "compute_price")[0]?.p.microLamports;
  if (price === undefined) return res("priority-fee", "Priority fee", "pass", "No priority fee set");
  const units = BigInt(parsedOf(ctx, "compute_limit")[0]?.p.units ?? 1_400_000);
  const fee = (price * units + 999_999n) / 1_000_000n;
  const max = BigInt(ctx.policy.limits.maxPriorityFeeLamports);
  if (fee > max) return res("priority-fee", "Priority fee", "fail", `Priority fee up to ${fee} lamports exceeds limit ${max}`);
  return res("priority-fee", "Priority fee", "pass", `Up to ${fee} lamports`);
}

export function checkIntentMatch(ctx: CheckContext): CheckResult {
  const intent = ctx.intent;
  if (!intent) return res("intent-match", "Matches declared intent", "skip", "No intent supplied");
  const { policy } = ctx;
  const problems: string[] = [];
  ...
  if (intent.action === "transfer") {
    ...
    if (!asset) return res("intent-match", "Matches declared intent", "fail", `Asset ${intent.asset} is not allowlisted`);
    const raw = parseUnits(intent.amount, asset.decimals);
    if (swaps.length) problems.push("Transaction contains a swap the intent did not ask for");
    if (asset.native) {
      const hits = solTransfers.filter(({ p }) => p.to === intent.to && p.lamports === raw);
      if (hits.length !== 1) problems.push(`Expected exactly one transfer of ${intent.amount} SOL to ${intent.to}`);
      if (out !== raw) problems.push(`SOL outflow ${sol(out)} differs from intent ${intent.amount} SOL`);
      if (tokenTransfers.length) problems.push("Unexpected token transfer");
    } else {
      const hits = tokenTransfers.filter(
        ({ p }) =>
          p.mint === asset.mint &&
          p.amount === raw &&
      ...
      if (out > 0n) problems.push(`Unexpected SOL outflow ${sol(out)}`);
    }
  } else if (intent.action === "swap") {
    const inp = resolveAsset(policy, intent.input);
    const outp = resolveAsset(policy, intent.output);
    if (!inp || !outp) return res("intent-match", "Matches declared intent", "fail", "Swap asset is not allowlisted");
    const raw = parseUnits(intent.amount, inp.decimals);
    if (swaps.length === 0) problems.push("Transaction contains no swap instruction");
    for (const { p } of swaps) {
      const spend = p.mode === "exact_in" ? p.amount : p.otherAmount;
      if (spend > raw) problems.push(`Swap spends up to ${formatUnits(spend, inp.decimals)} ${inp.symbol}, more than intended ${intent.amount}`);
      if (p.sourceMint && p.sourceMint !== inp.mint) problems.push(`Swap input token ${p.sourceMint} differs from intent (${inp.symbol})`);
      if (p.destinationMint && p.destinationMint !== outp.mint) problems.push(`Swap output token ${p.destinationMint} differs from intent (${outp.symbol})`);
    ...
    if (out > allowance) problems.push(`SOL outflow ${sol(out)} exceeds intent plus rent tolerance`);
    if (tokenTransfers.length) problems.push("Unexpected direct token transfer inside a swap");
  }
  if (problems.length) return res("intent-match", "Matches declared intent", "fail", problems[0], problems);
...
export function checkInjection(context: UntrustedContext | undefined, policy: Policy): InjectionOutcome {
  ...
  if (high.length) {
    check = res(
      ...
      policy.injection.mode === "block" ? "fail" : "warn",
      `Untrusted content contains wallet-targeted instructions (${high.map((f) => f.rule).join(", ")})`,
      details,
      true,
    );
...
export function outsideAddresses(ctx: CheckContext): string[] {
  const out = new Set<string>();
  const consider = (a?: string | null) => {
    if (a && a !== ctx.self && !ctx.selfTokenAccounts.has(a) && !ctx.allowed.has(a)) out.add(a);
  };
  for (const d of ctx.decoded) {
    const p = d.parsed;
    if (p.type === "sol_transfer") consider(p.to);
    else if (p.type === "token_transfer") consider(p.destination);
    else if (p.type === "token_approve") consider(p.delegate);
    else if (p.type === "authority_change") consider(p.newAuthority);
    else if (p.type === "token_close") consider(p.destination);
    else if (p.type === "assign") consider(p.owner);
    else if (p.type === "swap_route") consider(p.destination);
  }
  return [...out];
}
...
export function checkProvenance(
  suspects: string[],
  context: UntrustedContext | undefined,
  allowed: Set<string>,
  self: string,
): CheckResult {
  const texts = context?.untrusted ?? [];
  if (texts.length === 0) return res("provenance", "Destination provenance", "skip", "No untrusted context supplied");
  const trusted = context?.trusted ?? "";
  const tainted = suspects.filter(
    (a) => a !== self && !allowed.has(a) && texts.some((t) => t.includes(a)) && !trusted.includes(a),
  );
  if (tainted.length) {
    return res(
      "provenance",
      ...
      "fail",
      `Address ${shortAddr(tainted[0])} appears only in untrusted tool output`,
      tainted.map((a) => `${a} was taken from untrusted content and is not allowlisted`),
      true,
    );
  }
...
export async function runSimulation(
  ...
  rpc: Rpc | undefined,
  wire: Uint8Array,
): Promise<{ check: CheckResult; summary: SimulationSummary }> {
  if (!rpc) {
    return {
  ...
  try {
    ...
    const allowance =
      parseUnits(ctx.policy.limits.maxSolPerTx, 9) +
