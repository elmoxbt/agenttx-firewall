import type { SwapIntent, TransferIntent } from "./intent/intent.js";
import { resolveAsset, type Policy } from "./policy/policy.js";
...
/** Returns a serialized, unsigned transaction for a swap. The result is *untrusted* and is re-inspected. */
...
export async function buildTransfer(intent: TransferIntent, policy: Policy, self: string, rpc: Rpc): Promise<Uint8Array> {
  const asset = resolveAsset(policy, intent.asset);
  if (!asset) throw new BuildError(`Asset ${intent.asset} is not allowlisted`);
  const amount = parseUnits(intent.amount, asset.decimals);
  if (amount === 0n) throw new BuildError("Amount must be greater than zero");
  ...
  if (asset.native) {
    ...
    if (mintInfo) {
      if (!TOKEN_PROGRAMS.includes(mintInfo.owner)) throw new BuildError(`${asset.mint} is not a token mint`);
...
export async function buildSwap(intent: SwapIntent, policy: Policy, self: string, provider: SwapProvider): Promise<Uint8Array> {
  const inp = resolveAsset(policy, intent.input);
  const out = resolveAsset(policy, intent.output);
  if (!inp || !out) throw new BuildError("Swap asset is not allowlisted");
