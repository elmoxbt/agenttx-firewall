...
class Reader {
    ...
    u8() {
        if (this.o >= this.b.length)
            throw new Error("Unexpected end of transaction data");
...
export function parseTransaction(wire) {
    ...
    for (let i = 0; i < sigCount; i++)
        signatures.push(new Uint8Array(r.take(64)));
    const msgStart = r.o;
    let version = "legacy";
    let first = r.u8();
    if (first & 0x80) {
    ...
    for (let i = 0; i < nKeys; i++)
        staticAccountKeys.push(encodeBase58(r.take(32)));
    ...
    if (r.o !== wire.length)
        throw new Error(`Trailing bytes after transaction message (${wire.length - r.o})`);
    if (signatures.length !== header.numRequiredSignatures) {
        throw new Error(`Signature count (${signatures.length}) does not match required signatures (${header.numRequiredSignatures})`);
    }
...
export function requiredSigners(tx) {
...
export function resolveInstructions(tx, loaded) {
    ...
    const meta = (idx) => {
        if (idx >= keys.length)
            throw new Error(`Account index ${idx} is out of range`);
        let isSigner = false;
    ...
    return tx.compiledInstructions.map((ci) => {
        if (ci.programIdIndex >= keys.length)
            throw new Error(`Program index ${ci.programIdIndex} is out of range`);
        return { programId: keys[ci.programIdIndex], accounts: ci.accountIndexes.map(meta), data: ci.data };
    });
}
export function hasLookups(tx) {
    return tx.addressTableLookups.length > 0;
}
/** Address lookup table account layout: 56-byte header followed by 32-byte addresses. */
export function parseLookupTableAddresses(data) {
    const META = 56;
    if (data.length < META || (data.length - META) % 32 !== 0)
        throw new Error("Malformed address lookup table account");
    const out = [];
    for (let o = META; o < data.length; o += 32)
        out.push(encodeBase58(data.subarray(o, o + 32)));
    return out;
}
export function applyLookups(tx, tables) {
    const writable = [];
    const readonly = [];
    for (const l of tx.addressTableLookups) {
        const addrs = tables.get(l.accountKey);
        if (!addrs)
            throw new Error(`Lookup table ${l.accountKey} was not provided`);
        for (const i of l.writableIndexes) {
            if (i >= addrs.length)
                throw new Error(`Lookup index ${i} out of range for table ${l.accountKey}`);
...
export function compileLegacyMessage(feePayer, recentBlockhash, instructions) {
    const map = new Map();
    const add = (pubkey, signer, writable) => {
        ...
        if (e) {
            ...
            map.set(pubkey, { pubkey, signer, writable });
    };
    add(feePayer, true, true);
    for (const ix of instructions) {
        for (const a of ix.accounts)
            add(a.pubkey, a.isSigner, a.isWritable);
    ...
    if (ordered.length > 256)
        throw new Error("Too many accounts for a legacy transaction");
    ...
    for (const ix of instructions) {
        parts.push(Uint8Array.of(index.get(ix.programId)), Uint8Array.from(encodeShortVec(ix.accounts.length)), Uint8Array.from(ix.accounts.map((a) => index.get(a.pubkey))), Uint8Array.from(encodeShortVec(ix.data.length)), ix.data);
    }
    return concat(...parts);
}
...
export function withSignature(wire, signerIndex, signature) {
