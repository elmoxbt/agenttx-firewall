 ...
 */
export function isOnCurve(pub) {
    if (pub.length !== 32)
        return false;
    let y = 0n;
    for (let i = 31; i >= 0; i--)
        y = (y << 8n) | BigInt(i === 31 ? pub[i] & 0x7f : pub[i]);
    if (y >= P)
        return false;
    const y2 = (y * y) % P;
    const u = mod(y2 - 1n);
...
export function findProgramAddress(seeds, programId) {
    ...
    for (let bump = 255; bump >= 0; bump--) {
        ...
        if (!isOnCurve(out))
            return { address: encodeBase58(out), bump };
    }
...
export function deriveAta(owner, mint, tokenProgram = TOKEN_PROGRAM) {
    ...
    if (hit)
        return hit;
    const { address } = findProgramAddress([decodeBase58(owner), decodeBase58(tokenProgram), decodeBase58(mint)], ATA_PROGRAM);
