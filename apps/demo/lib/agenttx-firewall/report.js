...
import { decimalsForMint, symbolForMint } from "./policy/policy.js";
import { INFRA_PROGRAMS } from "./solana/constants.js";
import { describeInstruction } from "./solana/decode.js";
...
export function computeVerdict(checks) {
    ...
    if (fails.length)
        return { verdict: "BLOCK", risk: fails.some((c) => c.critical) ? "CRITICAL" : "HIGH" };
    if (warns.length)
        return { verdict: "REVIEW", risk: "MEDIUM" };
    return { verdict: "ALLOW", risk: "LOW" };
}
function amountLabel(policy, mint, raw) {
    if (!mint)
...
export function buildSummary(ctx) {
    ...
    const find = (t) => decoded.find((d) => d.parsed.type === t)?.parsed;
    ...
    if (swap) {
        ...
        if (names.length)
            summary.action = names.join(", ");
    }
    return summary;
}
export function summarizeInstructions(decoded) {
...
const TAG = { pass: "PASS", warn: "WARN", fail: "FAIL", skip: "SKIP" };
const COLOR = { pass: "32", warn: "33", fail: "31", skip: "90" };
export function renderReport(r, opts = {}) {
    const c = (code, s) => (opts.color ? `\u001b[${code}m${s}\u001b[0m` : s);
    const lines = [c("1", "AGENTTX SECURITY REPORT"), ""];
    const s = r.summary;
    ...
    if (s.recipient)
        lines.push(`Recipient: ${s.recipient}`);
    lines.push("");
